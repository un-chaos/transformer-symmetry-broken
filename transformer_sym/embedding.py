...
The embedding is the *first* place a global rotation of the ``d_model`` space can
be broken, so the embedding bias :class:`~transformer_sym.bias.EmbeddingBias`
lives here.  With ``b = 0`` the embedding is equivariant under ``x -> R x`` for
any orthogonal ``R``; with ``b != 0`` it is not, because ``R (x + b) = Rx + Rb``
and ``Rb != b`` unless ``R`` fixes ``b``.

Order of operations (fixed by the interface contract, do not reorder):

1. ``x = wte(tokens)``
2. ``x = x * sqrt(d_model)``  (only when ``scale``)
...
def sinusoidal_positions(max_seq_len: int, d_model: int) -> torch.Tensor:
    """Classic fixed sinusoidal position table, shape ``(max_seq_len, d_model)``.
    ...
    ``sqrt(d_model)``-scaled token embedding.
    """
    if max_seq_len < 1:
...
class TokenPositionalEmbedding(nn.Module):
    """Embedding of ``(B, T)`` token ids into ``(B, T, d_model)``.

    Args:
        vocab_size: number of token ids the table covers.
        d_model: embedding width.
        max_seq_len: length of the learned positional table / sinusoid table.
        dropout: dropout probability applied to the final embedding.
        scale: multiply the token embedding by ``sqrt(d_model)`` (Vaswani et al.).
        bias: optional :class:`~transformer_sym.bias.EmbeddingBias`; when given,
            ``x = x + b`` is applied *after* the positional term and *before*
            dropout.  This is the symmetry-breaking bias ``b``.
    ...
    def __init__(
        ...
        self.embed_scale = math.sqrt(self.d_model) if self.scale else 1.0

        self.wte = nn.Embedding(self.vocab_size, self.d_model)
        if self.learned_positional:
            self.wpe: Optional[nn.Embedding] = nn.Embedding(self.max_seq_len, self.d_model)
            self.register_buffer("_pos_table", None, persistent=False)
        else:
            self.wpe = None
            self.register_buffer(
                "_pos_table",
                sinusoidal_positions(self.max_seq_len, self.d_model),
                persistent=False,
            )
    ...
    def _positions(self, length: int, device: torch.device) -> torch.Tensor:
        """Return the positional term ``(T, d_model)`` for a sequence of ``length``."""
        if length > self.max_seq_len:
            raise ValueError(
                f"sequence length {length} exceeds max_seq_len {self.max_seq_len}; "
                "raise model.max_seq_len or clip the data"
            )
        if self.learned_positional:
    ...
    def extra_repr(self) -> str:
        return (
            ...
            f"max_seq_len={self.max_seq_len}, scale={self.scale}, "
