...
``AttentionBiasConfig.apply_decoder_self`` / ``apply_decoder_cross`` can switch
them on separately, and so ``share_across_layers`` can share one object within a
site while the two sites stay distinct.

Same development rule as :mod:`transformer_sym.encoder`: layers are **built in a
loop** and never ``copy.deepcopy``-ed, otherwise a shared ``AttentionBias`` would
be cloned per layer and ``share_across_layers`` would stop working.
"""
...
class DecoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.self_attn = MultiHeadAttention(
            d_model, n_heads, dropout=attention_dropout, bias=self_attn_bias
        )
        self.cross_attn = MultiHeadAttention(
            d_model, n_heads, dropout=attention_dropout, bias=cross_attn_bias
        )
        self.ff = FeedForward(
            d_model,
            d_ff,
...
class Decoder(nn.Module):
    ...
    Args:
        ...
        num_layers: defaults to ``cfg.n_decoder_layers``.
    ...
    def __init__(
        ...
        super().__init__()
        self.cfg = cfg
        n_layers = int(num_layers if num_layers is not None else cfg.n_decoder_layers)
        if n_layers < 1:
